...
功能：视觉搜索。先调用支持识图的 AI，将图片转化为 10 个以内关键词，关键词主要包含商品名字、外观、用途、特征；再把关键词拼成查询文本，调用 core/labelDB.py 中的 prod_search 搜索商品。
user_id: 用户的唯一字符串标识；传 "-1" 表示不记录用户标签。
image_path_or_url: 图片 URL、本地绝对路径或本地相对路径。
limit: 搜索返回的最大商品数。
返回值: dict，包含 keywords、query、product_ids。
HTTP 接口: POST /internal/v1/ai/search/image
请求示例:
{"user_id":"u1","image_path_or_url":"https://example.com/a.jpg","limit":20}
