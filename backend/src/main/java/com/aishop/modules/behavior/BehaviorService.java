...
public class BehaviorService {
    ...
    public BehaviorEventResponse recordEvent(String userId, BehaviorEventRequest request) {
        ...
        // 【修复】targetId 可为 null（如 SEARCH 事件），使用 null 安全的方式处理
        // 数据库 product_id 列为 VARCHAR(64)，直接存储字符串
        logEntity.setProductId(request.targetId());

        // 【修复 v3】metadata 处理：request.metadata() 是 Map<String, Object>
        // 直接设置到实体，Hibernate 会自动序列化为 JSONB
        Map<String, Object> metadata = request.metadata();
        if (metadata != null && !metadata.isEmpty()) {
            logEntity.setMetadata(metadata);
        }

        // 【修复 v3】如果是 SEARCH 事件，从 metadata Map 中提取 keyword 作为 query
        if ("SEARCH".equals(eventType) && metadata != null && !metadata.isEmpty()) {
